...
//    edm::TriggerNames const& triggerNames = iEvent.triggerNames(*hltresults);
    ...
    if (HltEvtCnt==0){
	  triggerNames = iEvent.triggerNames(*hltresults);
	  int firstevttriggersize = hltresults->size();
	  if (firstevttriggersize==0){std::cout << "%HLTInfo -- No trigger name given in TriggerResults of the input " << std::endl;}

	  ntrigs = dummyBranches_.size();

	  cout << "size of trigger list we will dump: "  << ntrigs << endl;
      for (unsigned int itrig = 0; itrig != ntrigs; ++itrig) {
        TString trigName = dummyBranches_[itrig].data();
...
//      cout << "itrig **********" << itrig << endl;
      ...
      std::string trigName = dummyBranches_[itrig].data();
	  unsigned int index = hltConfig_.triggerIndex(trigName);
...
//		  cout << " trigger missing   " << endl;
      ...
      bool accept = hltresults->accept(index);

//      cout << " Filling decision itrig  :  " << itrig << "      trigName:     " << trigName  << "            index : " << index << endl;
